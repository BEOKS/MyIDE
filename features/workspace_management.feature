Feature: Workspace management
  Scenario: Sessions map to app windows while windows map to LNB entries
    ...
    And the first session should start with 1 LNB windows
    And the first session LNB should include window title "Main"
    And adding a window should make the first session show 2 LNB windows
    And the first session LNB should include window title "Editor"
    And the second session should map to 2 app windows
    And the second session should start with 1 LNB windows
